...
class TiltingTableEnv(gym.Env):
    ...
    CENTERING_REWARD_SCALE = 2.0 # Sharpness of reward fall-off (tune this)
    Z_THRESHOLD = 0.08 # Use lower threshold

    MAX_STEPS_PER_EPISODE = 1000
    ACTION_PENALTY_WEIGHT = 0.001 # Use increased penalty weight from previous suggestion

    # Define expected joint names
    JOINT_X_NAME = 'joint_x'
    JOINT_Y_NAME = 'joint_y'
    ...
    def __init__(self):
        ...
        # --- Observation Space (MODIFIED) ---
        
        # Observation: [rel_ball_pos_x, y, rel_ball_vel_x, y, j_angle_x, y, j_vel_x, y]
        
        obs_limits_high = np.array([
            self.BOUND_LIMIT, self.BOUND_LIMIT, # rel pos x, y (use BOUND_LIMIT from termination check)
            5.0, 5.0,                            # rel vel x, y (adjust if needed)
            0.5, 0.5,                            # joint angle x, y (from SDF limit)
            5.0, 5.0                             # joint vel x, y (from SDF limit)
        ], dtype=np.float32)
        obs_limits_low = -obs_limits_high
        # Ensure ball position bounds match termination exactly
        obs_limits_low[0:2] = -self.BOUND_LIMIT
        obs_limits_high[0:2] = self.BOUND_LIMIT

        # Create the observation space with shape (8,)
        self.observation_space = spaces.Box(low=obs_limits_low, high=obs_limits_high, shape=(8,), dtype=np.float32)
        rospy.loginfo(f"Observation Space Updated: {self.observation_space}")
    ...
    def step(self, action):
        ...
        effort_req.duration = rospy.Duration(0.05)  # Continuous until next step command
        ...
        # ------------------------------------
        try:
            effort_req.joint_name = 'joint_x'
            effort_req.effort = effort_x
            self.apply_effort_proxy(effort_req)
            effort_req.joint_name = 'joint_y'
            effort_req.effort = effort_y
            self.apply_effort_proxy(effort_req)
        ...
        # Extract the ball state from current observation (excluding table tilt)
        observation_base = np.clip(
            self.current_observation, 
            self.observation_space.low[:-1], 
            self.observation_space.high[:-1]
        )
        rospy.logdebug(f"Base observation (ball state): {observation_base}")

        # 3. Compute the table tilt from the current plate pose
        table_tilt = self.compute_table_tilt()
        rospy.logdebug(f"Computed table tilt: {table_tilt:.3f}")

        # 4. Form the full observation (ball state + table tilt)
        observation = np.concatenate([observation_base, np.array([table_tilt])])
        observation = np.clip(observation, self.observation_space.low, self.observation_space.high)
        rospy.logdebug(f"Full observation (ball + tilt): {observation}")

        # Extract ball state for reward computation
        rel_x = observation[0]
        rel_y = observation[1]
        
        # 5. Calculate Reward (NEW Centering Reward Logic)
        ...
        reward = centering_reward + action_penalty
        rospy.logdebug(f"Reward components -> Centering: {centering_reward:.3f}, Action penalty: {action_penalty:.3f}, Total: {reward:.3f}")
        ...
        rospy.logdebug(f"Step {self.step_count}: Action: {action}, Observation: {observation}, Reward: {reward:.3f}, Terminated: {terminated}, Truncated: {truncated}")
        return observation, reward, terminated, truncated, info

    def compute_table_tilt(self):
        """Computes the table's tilt angle from its current pose.
           Returns 0.0 if the pose is not available."""
        if self.plate_pose_world is None:
            rospy.logwarn("Plate pose world is None, returning tilt 0.0")
            return 0.0
        q = self.plate_pose_world.orientation
        # Using a basic approximation: for small tilts, the angle can be approximated as 2*acos(q.w)
        tilt = 2 * math.acos(q.w)
        rospy.logdebug(f"Table tilt computed from quaternion (w={q.w:.3f}): {tilt:.3f}")
        return tilt
    ...
    def reset(self, seed=None, options=None):
        ...
        # Define table state: force it to a level pose (tilt = 0.0)
        table_state = ModelState(
            model_name='table_1',
            pose=Pose(
                position=Point(0, 0, 0),
                orientation=Quaternion(0, 0, 0, 1)  # level orientation => tilt 0.0
            ),
            twist=Twist()
        )
        initial_twist = Twist()
        ...
        effort_req.duration = rospy.Duration(-1)
        ...
        if not success:
            ...
            # Ensure the observation size matches the observation space, including the tilt.
            self.current_observation = np.zeros(self.observation_space.shape[0] - 1, dtype=np.float32)

        # Add the table tilt (which is 0.0 after reset) to the observation vector.
        table_tilt = 0.0
        # If self.current_observation already has table info, replace the following line with your extraction logic.
        full_observation = np.concatenate([self.current_observation, np.array([table_tilt])])
        observation = np.clip(full_observation, self.observation_space.low, self.observation_space.high)
        info = {}
        self.step_count = 0
